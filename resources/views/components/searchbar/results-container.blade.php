...
<div
    x-data="{
        ...
        open: function (event) {
            this.isOpen = true;
            this.width = $refs.searchInputWrapper.offsetWidth;
        },
    ...
    {{
        $attributes->class([
            'fi-global-search-results-ctn absolute inset-x-4 z-10 mt-2 max-h-96 overflow-auto rounded-lg bg-white shadow-lg ring-1 ring-gray-950/5 transition dark:bg-gray-900 dark:ring-white/10 sm:inset-x-auto sm:w-screen',
    ...
    @else
        <ul class="divide-y divide-gray-200 dark:divide-white/10">
            @foreach ($results->getCategories() as $group => $groupedResults)
                @include('components.searchbar.result-group', ['label' => $group, 'results' => $groupedResults])
            @endforeach
        </ul>
    @endif
</div>
